...
class ChartViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        if UIDevice().model == "iPad" {
            ...
            if ( blackChart ) {
                addBlackAxis(BarsToShow: 150)
            } else { addWhiteAxis(BarsToShow: 150) }
        } else {
            
            if ( blackChart ) {
                addBlackAxis(BarsToShow: 75)
            } else { addWhiteAxis(BarsToShow: 75) }
        }
        addDefaultModifiers()
        addWhiteDataSeries()
...
//        }
    ...
    fileprivate func addBlackSurface() {
        surface = SCIChartSurface(frame: self.chartView.bounds)
        surface.translatesAutoresizingMaskIntoConstraints = true
        surface.frame = self.chartView.bounds
        surface.autoresizingMask = [.flexibleHeight, .flexibleWidth]
        ...
        surface.renderableSeriesAreaFill.color = .black
        surface.renderableSeriesAreaBorder = SCISolidPenStyle(color: .black, withThickness: 1)
        self.view.addSubview(surface)
    }
    ...
    fileprivate func addBlackAxis(BarsToShow: Int) {
        ...
        // this made some of the backgroung white
        xAxis.style.gridBandBrush = SCISolidBrushStyle(color: .black)
        xAxis.style.drawMajorBands = true //-- this did not render a white background
        
        
        // Date changing major grid line color and thicknes. major grid line is line at the label position
        xAxis.style.majorGridLineBrush = SCISolidPenStyle(color: .darkGray, withThickness: 0.5)  // date vertical lines
        
        // changing minor grid line color and thicknes. minor grid lines are located between major grid lines -- ??
//xAxis.style.minorGridLineBrush = SCISolidPenStyle(color: .black, withThickness: 0.5)
        // date axis label color
        xAxis.style.labelStyle.color = .white
        // axis label font
        xAxis.style.labelStyle.fontName = "Helvetica"
        // axis label font size
        xAxis.style.labelStyle.fontSize = 14
...
xAxis.style.drawMinorTicks = false
        ...
        // major ticks are marks on axis that are located at label
        // length of major tick in points
        xAxis.style.majorTickSize = 10
        
        // color and thicknes of major tick - On the Date axis
        //xAxis.style.majorTickBrush = SCISolidPenStyle(color: .darkGray, withThickness: 1) // date ticks
        
        // minor ticks are marks on axis that fills space between major ticks
        // length of minor tick in points
        xAxis.style.minorTickSize = 2
        // color and thicknes of minor tick  -- ??
        xAxis.style.minorTickBrush = SCISolidPenStyle(color: .yellow, withThickness: 1) // unknown
        surface.xAxes.add(xAxis)
...
// Price
        ...
        // setting axis band color. Band is filled area between major grid lines
yAxis.style.gridBandBrush = SCISolidBrushStyle(color: .black)
        yAxis.style.drawMajorBands = true //- this did not render a white background
        
        // changing major grid line color and thicknes. major grid line is line at the label position//
//yAxis.style.majorGridLineBrush = SCISolidPenStyle(color: .darkGray, withThickness: 0.5)    // major price lines
        
        // changing minor grid line color and thicknes. minor grid lines are located between major grid lines
// yAxis.style.minorGridLineBrush = SCISolidPenStyle(color: .red, withThickness: 0.5)  // minor price lines - un needed clutter
        
        // set custom label provider for axis. Label provider defines text for labels
        //yAxis.labelProvider = ThousandsLabelProvider()
        // axis label color
        yAxis.style.labelStyle.color = .white
        // axis label font size
        yAxis.style.labelStyle.fontSize = 12
        // major ticks are marks on axis that are located at label
        // length of major tick in points
        yAxis.style.majorTickSize = 1
        
        // color and thicknes of major tick
        yAxis.style.majorTickBrush = SCISolidPenStyle(color: .black, withThickness: 0.5)
        yAxis.style.drawMinorTicks = false
        // minor ticks are marks on axis that fills space between major ticks
        // length of minor tick in points
        //yAxis.style.minorTickSize = 2
        // color and thicknes of minor tick
        yAxis.style.minorTickBrush = SCISolidPenStyle(color: .red, withThickness: 0.5)
